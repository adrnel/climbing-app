...
router.post('/api/archscores/', function(req, res) {
  ...
  connection.query("INSERT INTO `" + process.env.DATABASE + "`.`arch_scores` (`user_id`, `spotty`, `black`, `tiger`, `blue`, `salmon`, `yellow`, `yurple`, `hendrix`, `red`, `white`, `green`, `score`, `score_date`) VALUES (" + req.session.user.user_id + ", " + spotty + ", " + black + ", " + tiger + ", " + blue + ", " + salmon + ", " + yellow + ", " + yurple + ", " + hendrix + ", " + red + ", " + white + ", " + green + ", " + score + ", STR_TO_DATE('"+score_date+"', '%Y-%m-%d'))", function(err, rows, fields) {
    if (err) {
      console.log('this.sql', this.sql); //command/query
      console.error(err);
      res.status(404);
